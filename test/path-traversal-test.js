...
const R11 = CORE_RULES.find(r => r.id === 'R11');
...
test("[NEG] runEngine readlink path returns ALLOW", () => {
  const res = executeEngine('core', 'local CHROME_SRC="$(readlink -f \\"${SCRIPT_DIR}/../../\\")"');
  assert.strictEqual(res.verdict, 'ALLOW');
});

test("[POS] runEngine cat /etc/passwd returns WARN (score 80 >= 60)", () => {
  const res = executeEngine('core', 'cat ../../../etc/passwd');
  assert.strictEqual(res.verdict, 'WARN');
  assert.strictEqual(res.highestRule, 'R11');
});
