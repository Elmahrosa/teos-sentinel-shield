...
const { renderHtmlReport } = require('../../reporting/report-renderer');
...
// Replay verification (HTML report)
router.get('/report/:auditId', (req, res) => {
  const record = auditStore.get(req.params.auditId);
  if (!record) {
    return res.status(404).json({ error: 'audit_not_found', message: 'No audit record for this ID' });
  }

  const report = buildReport(record, record.dataSources || buildEmptyDataSources());
